...
// port used for updating cursor
#define CRTC_PORT         0x3D4
// port codes for udpating upper and lower cursor position bits
#define CUR_POS_CODE_L    0x0F
#define CUR_POS_CODE_U    0x0E
#define CUR_FF_MASK       0xFF
#define R_SHIFT_OFFSET    8
#define VGA_CHAR_BYTE_NUM 2
...
void test_interrupts(void); //for testing rtc_interrupt

/* Userspace address-check functions */
int32_t bad_userspace_addr(const void* addr, int32_t len);
int32_t safe_strncpy(int8_t* dest, const int8_t* src, int32_t n);

// max size of a buffered read
#define LINE_SIZE       128

// struct of the terminal variables
typedef struct terminal_t {
    uint8_t read_flag;
    uint32_t line_buf_idx;
    uint32_t read_length;
    uint8_t line_buf[LINE_SIZE];
    uint8_t tab_idx[LINE_SIZE];
    int32_t screen_x;
    int32_t screen_y;
    int32_t nl_offset;
    int8_t* video_mem;
